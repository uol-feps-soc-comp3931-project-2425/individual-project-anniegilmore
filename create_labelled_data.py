...
def format_dataset() -> None:
    ...
    for folder in ["test", "train"]:
        ...
        for lvl2_folder in level2_folders:
            images = [
                f
                for f in os.listdir(f"{DATA_PATH}/{folder}/{lvl2_folder}")
                if f.lower().endswith(".png")
            ]
            for image in images:
                label_data.append(
                    {"image": f"{folder}_{lvl2_folder}_{image}", "level": lvl2_folder}
                )
                shutil.copy(
                    Path(f"{DATA_PATH}/{folder}/{lvl2_folder}") / image,
                    Path(f"{DATA_PATH}/images/{folder}_{lvl2_folder}_{image}"),
                )

        with open(
            Path(f"../Cifar10/cifar10/{folder}Labels.csv"), "w", newline=""
        ) as csvfile:
